    ...
    <div class="col-md-12 justify-content-center row mt-5">
        ...
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    {!! Form::open([ 'route' => [ 'update-categories', $category->id ] ]) !!}
                                ...
                                <select name="catalog" class="form-control">
                                    @if(isset($categories_parent) > 0)
                                        @foreach($categories_parent as $category_parent)
                                            <option value="{{ $category_parent['id'] }}" @if($category_parent['id'] == $category->catalog->id) selected @endif>{{ $category_parent['name'] }}</option>
                                        @endforeach
                                    @endif
                                </select>
                            </div>
